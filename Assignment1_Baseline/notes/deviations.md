...
2. Changed sudo's default PATH to allow pipeline to run with required dependencies
(was missing Playwright/mitmproxy despite both being installed)
3. Edited general_functions.py (at HEAD) which references read_cov_from_file, 
...
4. Correspondingly, crawler/utils.py does not exist in the repository at all; 
...
5. DVWA's config.inc.php was reset to its .dist template on every docker compose up 
